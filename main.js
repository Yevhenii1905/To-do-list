...
function updatelocalStorage() {
    ...
    localStorage.setItem('tasks', JSON.stringify(tasks)); // Добавлено сохранение в LocalStorage
}

// Функция для загрузки задач из LocalStorage
function loadTasksFromLocalStorage() {
    const tasks = JSON.parse(localStorage.getItem('tasks')) || []; // Загружаем задачи из LocalStorage или пустой массив
    tasks.forEach(task => {
...
document.getElementById('clear-all').addEventListener('click', function(){
    ...
    if(confirmed) {
        ...
        localStorage.removeItem('tasks'); // Удаляем задачи из LocalStorage
    }
});